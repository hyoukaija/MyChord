...
public final class FingerTable {
	...
	private final int fingerTableSize = 202;
	 ...
	 */
	public FingerTable(int idSizeInByte, RoutingAlgorithm algorithm, IDAddressPair selfIDAddressPair,
			boolean aggressiveJoining) {
		this.idSizeInBit = idSizeInByte * 8;

		this.fingerTable = new IDAddressPair[this.fingerTableSize+1];
		//this.fingerTable = new IDAddressPair[200];
	...
	synchronized void clear() {
		...
		for (int i = 0; i < this.fingerTableSize+1; i++) {//table　size 変更した
	...
	public boolean put(IDAddressPair newEntry) {
		...
		BigInteger distanceOfNewEntry = algorithm.distance(newID, selfID);
		...
		int largestIndex = 0;
		for(int i = 101;i > 0; i--){
			if(distanceOfNewEntry.compareTo(THREE.pow(i)) == 1 ){
				if(distanceOfNewEntry.compareTo(THREE.pow(i).multiply(TWO)) == 1){
					i = 2*i+2;
				}else{
					i = 2*i+1;
				}
	...
	/**
	 * Put the specified entry to this finger table from finger[i] down to
	 * possibly finger[1].
	 *
	 * @return true if an entry was updated.
	 */
	public boolean put(IDAddressPair newEntry, int largestIndex) {
		...
		synchronized (this) {
			...
			if (updated) {
				logger.log(Level.INFO, "FingerTable#put: " + newEntry + " from " + largestIndex + " to " + (i + 1));
			}
		} // synchronized (this)
	...
	/**
	 * Remove the specified entry from this finger table and fill the blanks
	 * with the appropriate alternative entry.
	 */
	public void remove(ID target) {
		...
		if (possibleLargestIndex > this.fingerTableSize) {
	...
	public int numOfDifferentEntries() {
		...
		synchronized (this.fingerTable) {
			for (int i = this.fingerTableSize; i >= 1; i--) {
