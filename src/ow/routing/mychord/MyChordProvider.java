...
public class MyChordProvider implements RoutingAlgorithmProvider {
	...
	public RoutingAlgorithm initializeAlgorithmInstance(
		...
		boolean aggressiveJoiningMode = ((MyChordConfiguration)conf).getAggressiveJoiningMode();

		RoutingAlgorithm algo;
		if (aggressiveJoiningMode) {
			algo = new MyChordInAggressiveJoiningMode(conf, routingSvc);
		}
		else {
			algo = new MyChord(conf, routingSvc);
		}
